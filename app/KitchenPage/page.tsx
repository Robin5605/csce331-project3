    ...
    <li key={item.id} className="flex flex-col gap-1">
      ...
      {item.ingredients && item.ingredients.length > 0 && (
        <p className="text-[11px] text-neutral-400 ml-10">
          {item.ingredients
            .map(ing => `${ing.name} x${ing.servings}`)
            .join(" • ")}
                </p>
            )}
            </li>
      ...
      <div className="mt-2 flex items-center justify-between gap-2">
        ...
        >
          {isUpdating ? "Updating..." : getButtonLabel(order.order_status)}
        </button>
      </div>
    </div>
  );
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        const res = await fetch("/api/orders?order_status=not_working_on");
...
const handleAdvanceStatus = async (order: Order) => {
  ...
  try {
    ...
    if (!res.ok) {
      console.error("Failed to update order status", await res.json());
      return;
    }
    ...
    setOrders(prev =>
      prev
        .map(o =>
          o.id === order.id ? { ...o, order_status: updated.order_status } : o
        )
        // remove only when completed
        .filter(o => !(o.id === order.id && updated.order_status === "completed"))
    );
  } catch (err) {
    console.error(err);
  } finally {
      ...
      <main className="flex-1 w-full max-w-6xl mx-auto px-4 py-6">
        ...
        <div className="mb-4 flex items-center justify-between gap-4">
          ...
          <div className="text-sm text-neutral-300">
            {loading ? "Loading..." : `${orders.length} order(s) in queue`}
          </div>
        </div>
