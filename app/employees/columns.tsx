    ...
    return (
        <AlertDialog>
            <AlertDialogTrigger asChild>
                <Button variant="destructive" size="icon" className="cursor-pointer"><X/></Button>
            </AlertDialogTrigger>
            <AlertDialogContent>
                <AlertDialogHeader>
                    <AlertDialogTitle>Remove {employee.name}</AlertDialogTitle>
                    <AlertDialogDescription>
                    Are you sure you want to remove {employee.name} from the records? This action cannot be undone.
                    </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
    ...
    return (
        <Dialog>
            <DialogTrigger asChild>
                <Button variant="outline" size="icon" className="cursor-pointer"><Pencil /></Button>
            </DialogTrigger>
            <DialogContent>
                <DialogHeader>
                ...
                <div className="grid w-full max-w-sm items-center gap-3">
                    ...
                    <span className="flex gap-2 w-full">
                        <Input 
                            ...
                            className="flex-1"
                        />
                        <Button variant="outline" size="icon" onClick={generatePin}><RefreshCcw /></Button>
                    </span>
                </div>
                <DialogFooter>
                    <DialogClose asChild>
                        <Button variant="outline" onClick={() => setState(employee)}>Cancel</Button>
                    </DialogClose>
                    <DialogClose asChild>
                        <Button type="submit" onClick={async () => {
                            const newEmployeeData = await updateEmployeeAction(employee.id, name, hoursWorked, pin);
